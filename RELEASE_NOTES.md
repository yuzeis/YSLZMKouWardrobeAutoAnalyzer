# YSLZMKouWardrobeAutoAnalyzer ver1.1

版本代号：Gnadenfülle

这是首个稳定正式版本。程序面向 Windows PC 版《以闪亮之名》，通过
Scapy 与 Npcap 被动读取本机游戏流量，生成衣柜与卡池数据报告以及微信小程序
导入内容。程序不抽卡、不注入或重放请求、不操作微信，也不上传数据。

## 本次更新

- 同步抠池记录当前卡池目录：支持 203 个普通池及 126 个背景池；导入码和 C1 紧凑目录已一并更新。
- 氪背只依据完整命令 740 中直接对应场景 `lock_id` 的 `tmp_id` 判定；命令 637 的头像/相框 ID 与旧 `unlock_way` 目标均明确排除，快照不完整时失败关闭。
- 不再接受调用方传入的背景所有权映射；损坏的命令 740 不能绕过抓包证据门槛。
...
- 使用权威登录抓包连续执行 20 轮确定性模拟；每轮均在 5 秒目标内完成，并与 Android 原始 JSON 逐字节一致。
...
- `YSLZMKouWardrobeAutoAnalyzer-ver1.1-windows-x64.exe`：Windows x64 单文件程序。
- `YSLZMKouWardrobeAutoAnalyzer-ver1.1-source.zip`：与标签对应的完整源码。
